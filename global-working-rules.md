...
- Temporary work must stay inside the current project root’s `.tmp/` directory. Never write temporary files to the system `/tmp` directory or to any other path outside the current project root. Create `.tmp/` when needed; in Git-managed projects, create or update the project `.gitignore` to include `.tmp/` so temporary work remains untracked. In non-Git folders, use `.tmp/` without creating an ignore file.
